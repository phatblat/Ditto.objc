...
/**
 ...
 They allow chaining of further collections-related functions. You can either call `exec` on the object to get an array of `DITCollection`s as an
 immediate return value, or you can establish either a live query or a subscription, which both work over time.
...
/**
 ...
 This is useful if you aren't interested in the first N collections for one reason or another. For example, you might already have obtained the first 20
 collections and so you might want to get the next 20 collections, and that is when you would use `offset`.

 @param offset The number of collections that you want the eventual resulting set of collections to be offset by (and thus not include).
...
/**
 Enables you to subscribe to updates about collections that other devices know about, and have a block called when an update is received
 ...
 @param eventHandler A block that will be called every time there is an update about the list of known about collections.

 @return A `DITLiveQuery` object that must be kept in scope for as long as you want to keep
 receiving updates.
 */
- (DITLiveQuery *)observe:(void (^)(DITCollectionsEvent *))eventHandler;
...
/**
 ...
 @param dispatchQueue The dispatch queue that will be used to deliver updates. Defaults to the main queue.
 @param eventHandler A block that will be called every time there is an update about the list of known about collections.
...
/**
 Enables you to subscribe to updates about collections that other devices know about, and have a block called when an update is received
 either locally or remotely. You wont receive any further callbacks until you explicitly signal that you are ready for the next event to be delivered.
 ...
 receiving updates.
 */
- (DITLiveQuery *)observeWithNextSignal:(void (^)(DITCollectionsEvent *, DittoSignalNextBlock))eventHandler;
...
/**
 ...
 @param dispatchQueue The dispatch queue that will be used to deliver live query updates. Defaults to the main queue.
 @param eventHandler A block that will be called every time there is an update about the list of known about collections.
...
- (DITLiveQuery *)observeWithNextSignalAndDeliveryQueue:(dispatch_queue_t)dispatchQueue
                                           eventHandler:(void (^)(DITCollectionsEvent *, DittoSignalNextBlock))eventHandler NS_SWIFT_NAME(observeWithNextSignal(deliveryQueue:eventHandler:));

/**
 Enables you to subscribe to updates about collections local to this device.

 This won't subscribe to receive updates from other devices and so it will only fire when a local change to the known collections occurs.
 If you want to receive remote updates as well then use `observe` or call `subscribe`.
...
/**
 ...
 receiving updates.
 */
- (DITLiveQuery *)observeLocalWithNextSignal:(void (^)(DITCollectionsEvent *, DittoSignalNextBlock))eventHandler;
...
- (DITLiveQuery *)observeLocalWithNextSignalAndDeliveryQueue:(dispatch_queue_t)dispatchQueue
                                                eventHandler:(void (^)(DITCollectionsEvent *, DittoSignalNextBlock))eventHandler NS_SWIFT_NAME(observeLocalWithNextSignal(deliveryQueue:eventHandler:));
